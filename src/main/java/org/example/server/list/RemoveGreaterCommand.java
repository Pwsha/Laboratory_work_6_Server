...
public class RemoveGreaterCommand implements Command {
    ...
    public CommandResponse execute(CommandRequest request, CollectionManager manager, Scanner scanner, int userId) {
        ...
        if (reference == null) return CommandResponse.error("Не указан эталонный элемент");

        int initialSize = this.manager.getCollection().size();
        this.manager.getCollection().removeIf(g -> g.compareTo(reference) > 0);
        int removed = initialSize - this.manager.getCollection().size();
        return CommandResponse.success("Удалено элементов: " + removed);
    }

    @Override public String getName() { return "remove_greater"; }
    @Override public String getDescription() { return "удалить элементы, превышающие заданный"; }
    @Override public String getSyntax() { return "remove_greater {element}"; }
}