...
public class CommandHelper {
    ...
    public static LocalDateTime generateCreationDate() {
        return LocalDateTime.now();
    }

    // Полная подготовка StudyGroup для сохранения (ID + дата)
    public static StudyGroup prepareForSave(StudyGroup group, HashSet<StudyGroup> collection) {
        if (group.getId() == null) {
            group.setId(generateId(collection));
        }
        if (group.getCreationDate() == null) {
            group.setCreationDate(generateCreationDate());
        }
        return group;
    }
